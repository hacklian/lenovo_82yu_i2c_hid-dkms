...
_log_info "Downloading linux kernel files from repository..."
wget -q "https://git.kernel.org/pub/scm/linux/kernel/git/stable/linux.git/plain/drivers/hid/i2c-hid/i2c-hid-core.c?h=v${KERNEL_VERSION}" -O "${WORK_DIR}/i2c-hid-core.c" \
 && _log_done "Downloaded i2c-hid-core.c!" "  > " || _not_downloadable "i2c-hid-core.c" "  > "
wget -q "https://git.kernel.org/pub/scm/linux/kernel/git/stable/linux.git/plain/drivers/hid/i2c-hid/i2c-hid-dmi-quirks.c?h=v${KERNEL_VERSION}" -O "${WORK_DIR}/i2c-hid-dmi-quirks.c" \
 && _log_done "Downloaded i2c-hid-dmi-quirks.c!" "  > " || _not_downloadable "i2c-hid-dmi-quirks.c" "  > "
wget -q "https://git.kernel.org/pub/scm/linux/kernel/git/stable/linux.git/plain/drivers/hid/i2c-hid/i2c-hid.h?h=v${KERNEL_VERSION}" -O "${WORK_DIR}/i2c-hid.h" \
 && _log_done "Downloaded i2c-hid.h!" "  > " || _not_downloadable "i2c-hid.h" "  > "
wget -q "https://git.kernel.org/pub/scm/linux/kernel/git/stable/linux.git/plain/drivers/hid/hid-ids.h?h=v${KERNEL_VERSION}" -O "${WORK_DIR}/hid-ids.h" \
 && _log_done "Downloaded hid-ids.h!" "  > " || _not_downloadable "hid-ids.h" "  > "

_log_info "Patching downloaded kernel files..."
I2C_HID_CORE_PATCHFILE="patch/${KERNEL_VERSION}/i2c-hid-core.c.patch"
if [[ ! -f "${SOURCE_DIR}/${I2C_HID_CORE_PATCHFILE}" ]]; then
	_log_info "No i2c-hid-core.c patchfile found for kernel version ${KERNEL_VERSION}, trying generic patchfile..." "  > "
	I2C_HID_CORE_PATCHFILE="patch/i2c-hid-core.c.patch"
fi

patch --dry-run --quiet "${WORK_DIR}/i2c-hid-core.c" "${SOURCE_DIR}/${I2C_HID_CORE_PATCHFILE}" &> /dev/null \
 && patch --quiet "${WORK_DIR}/i2c-hid-core.c" "${SOURCE_DIR}/${I2C_HID_CORE_PATCHFILE}" && _log_done "Patched i2c-hid-core.c!" "  > " \
 ||  _not_patchable "i2c-hid-core.c"

if [[ "$(echo "${KERNEL_VERSION} >= 6.3" | bc)" == 1 ]]; then
	I2C_HID_DMI_QUIRKS_PATCHFILE="patch/${KERNEL_VERSION}/i2c-hid-dmi-quirks.c.patch"
	if [[ ! -f "${SOURCE_DIR}/${I2C_HID_DMI_QUIRKS_PATCHFILE}" ]]; then
		_log_info "No i2c-hid-dmi-quirks.c patchfile found for kernel version ${KERNEL_VERSION}, trying generic patchfile..." "  > "
		I2C_HID_DMI_QUIRKS_PATCHFILE="patch/i2c-hid-dmi-quirks.c.patch"
	fi

	patch --dry-run --quiet "${WORK_DIR}/i2c-hid-dmi-quirks.c" "${SOURCE_DIR}/${I2C_HID_DMI_QUIRKS_PATCHFILE}" &> /dev/null \
	 && patch --quiet "${WORK_DIR}/i2c-hid-dmi-quirks.c" "${SOURCE_DIR}/${I2C_HID_DMI_QUIRKS_PATCHFILE}" && _log_done "Patched i2c-hid-dmi-quirks.c!" "  > " \
	 ||  _not_patchable "i2c-hid-dmi-quirks.c"
fi
